...
def _fullwidth(text: str) -> str:
    """Digits and the plus in their fullwidth forms, U+FF10.. and U+FF0B, without ambiguous literals in the source."""
...
def test_acceptance_span() -> None:
    span = LinkDetector(phones=_US).find("Call 650-253-0000")[0]
    assert span == LinkSpan(
        5,
        17,
        "650-253-0000",
        "tel:+16502530000",
    ...
    assert span.phone.international_number == "+16502530000"
...
def test_has_link_sees_phones() -> None:
    assert LinkDetector(phones=_US).has_link("650-253-0000") is True
    assert LinkDetector().has_link("650-253-0000") is False
    assert LinkDetector(phones=_US).has_link("no number here") is False


def test_has_link_exits_early_on_a_long_tail() -> None:
    text = "650-253-0000 " + "tail " * 45_000
    assert LinkDetector(phones=_US).has_link(text) is True


def test_span_repr_shows_the_phone() -> None:
    span = LinkDetector(phones=_US).find("650-253-0000")[0]
    assert repr(span) == (
...
def test_spans_differing_only_in_phone_are_unequal() -> None:
    ...
    with_phone = LinkSpan(0, 12, "650-253-0000", "tel:+16502530000", is_email=False, phone=number)
    without = LinkSpan(0, 12, "650-253-0000", "tel:+16502530000", is_email=False)
    assert with_phone != without
...
def test_a_long_run_of_groups_is_bounded() -> None:
    text = " ".join(["1234"] * 30) + " 650-253-0000"
...
def test_readings(text: str, regions: tuple[str, ...], url: str, region: str, number_type: PhoneType) -> None:
    spans = LinkDetector(phones=PhoneNumbers(regions=regions)).find(text)
    assert [span.url for span in spans] == [url]
    assert spans[0].phone is not None
    assert (spans[0].phone.region, spans[0].phone.type) == (region, number_type)
...
def test_plus_only_mode_ignores_national_text() -> None:
    detector = LinkDetector(phones=PhoneNumbers())
    assert detector.find("650-253-0000 but +1 650-253-0001")[0].url == "tel:+16502530001"
    assert len(detector.find("650-253-0000 but +1 650-253-0001")) == 1


@pytest.mark.parametrize(
    ("text", "url", "region", "general"),
    [
        pytest.param("555-123-4567", "tel:+15551234567", None, False, id="possible-unrouted-without-a-type"),
        pytest.param("650-253-0000", "tel:+16502530000", "US", True, id="valid-number-still-unknown-type"),
        pytest.param("268 555 1234", "tel:+12685551234", "AG", False, id="routed-region-reported"),
    ],
)
def test_possible_mode(text: str, url: str, region: str | None, general: bool) -> None:  # ruff:ignore[boolean-type-hint-positional-argument]
    detector = LinkDetector(phones=PhoneNumbers(regions=("US",), require_valid=False))
    spans = detector.find(text)
    assert [span.url for span in spans] == [url]
    assert spans[0].phone is not None
    assert (spans[0].phone.region, spans[0].phone.type) == (region, PhoneType.UNKNOWN)
    assert bool(general) is (region == "US")


def test_possible_mode_rejects_impossible_lengths() -> None:
    detector = LinkDetector(phones=PhoneNumbers(regions=("US",), require_valid=False))
    assert detector.find("555-1234-56789") == []


def test_possible_mode_accepts_local_only_lengths() -> None:
    detector = LinkDetector(phones=PhoneNumbers(regions=("US",), require_valid=False))
    assert [span.url for span in detector.find("253-0000")] == ["tel:+12530000"]
