...
through :meth:`~turbohtml.Node.select`/:meth:`~turbohtml.Node.matches`/:meth:`~turbohtml.Node.closest`; this module
wraps those node methods in the
soupsieve call shapes so porting a soupsieve (or bs4 ``Tag.select``) codebase is a mechanical import swap rather than a
rewrite.

It is a pure-Python facade -- no second engine. :func:`compile` validates the selector once up front (soupsieve compiles
eagerly), raising :class:`SelectorSyntaxError` for a malformed selector, and the returned :class:`Matcher` re-runs the
native engine per call. The selector entry points on the C core take only the selector string, so the soupsieve
``namespaces`` and ``flags`` arguments are bundled into one immutable :class:`Matching` config that travels with the
matcher for API parity; they do not alter match *results* (mirroring soupsieve, whose ``flags`` are advisory and whose
namespace discrimination turbohtml does not yet apply -- see the module reference for the limitation).
"""
...
class Matching:
    ...
    def soupsieve(cls, namespaces: Mapping[str, str] | None = None, flags: int = 0) -> Matching:
        ...
        Lets a port keep the original argument list -- ``Matching.soupsieve(namespaces=ns, flags=DEBUG)`` -- rather than
        renaming keywords at every call site.
...
_DEFAULT = Matching()
# A throwaway element to validate selector syntax at compile time; matching against it parses the selector, raising on a
# malformed one, without needing the caller's node.
...
class Matcher:
    ...
    def select(self, node: Element, limit: int = 0) -> list[Element]:
        """
        Collect the descendants of an element that match the selector, in document order.

        :param node: the root whose descendants are searched.
    ...
    def select_one(self, node: Element) -> Element | None:
        """
    ...
    def iselect(self, node: Element, limit: int = 0) -> Iterator[Element]:
        """
    ...
    def filter(self, iterable: Element | Iterable[Element]) -> list[Element]:
        """
        Keep the members of an iterable that match the selector.

        :param iterable: elements to test, or one element whose direct element children are tested (soupsieve's rule).
        :returns: the members that match.
        """
        if isinstance(iterable, Element):
...
def select(selector: str, node: Element, options: Matching | None = None, /, *, limit: int = 0) -> list[Element]:
    """
...
def select_one(selector: str, node: Element, options: Matching | None = None, /) -> Element | None:
    """
...
def iselect(selector: str, node: Element, options: Matching | None = None, /, *, limit: int = 0) -> Iterator[Element]:
    """
...
def filter(  # noqa: A001  # the soupsieve entry-point name
    selector: str, iterable: Element | Iterable[Element], options: Matching | None = None, /
) -> list[Element]:
    """
    Keep the members of an iterable matching a selector, compiling it for this one call.

    :param selector: the CSS selector.
    :param iterable: elements to test, or one element whose direct element children are tested.
    :param options: the :class:`Matching` config, or ``None`` for soupsieve's defaults.
    :returns: the members that match.
    """
