...
.. testcode::
    ...
    print([a.attr("href") for a in links.select(doc.root)])
    print(links.select_one(doc.root).attr("href"))
...
.. testcode::
    ...
    anchor = links.select_one(doc.root)
...
.. testcode::
    ...
    print([a.attr("href") for a in match.select("a[href]", doc.root, limit=1)])
    print(match.match("a", match.select_one("a", doc.root)))
...
.. testcode::
    ...
    print(match.select_one(f"#{match.escape(raw_id)}", page.root).text)
