...
input at all, interleaved with the tokenizer's in source order. ``strict=True`` therefore raises on documents it used to
accept. Nothing gated the ``errors`` arrays the vendored ``html5lib-tests`` tokenizer suite carries; they now gate the
