...
37 to 93 times faster, except on the ``has_link`` prose row (2.1x), where ``test`` short-circuits on the first link near
the start.

.. bench-table::
    :file: bench/detect.json
...
The ``google_doc`` case reads the inline-CSS styling a Google Docs export carries (html2text's google_doc mode) and runs
56 times faster; markdownify has no equivalent.

.. bench-table::
    :file: bench/markdown-google.json
...
.. bench-table::
    :file: bench/path.json

The positional ``xpath_path`` races both ``getpath`` and parsel's ``getroottree().getpath()``, and leads each by the
same six-times margin.

.. bench-table::
    :file: bench/path-xpath.json
...
.. bench-table::
    :file: bench/text-render.json

The collapsed word stream turns layout guessing off: turbohtml joins the :attr:`~turbohtml.Node.stripped_strings` word
stream against html-text's ``extract_text(guess_layout=False)``, 48 times faster; inscriptis and resiliparse have no
comparable collapsed mode.

.. bench-table::
    :file: bench/text-collapsed.json

The main-content pass strips page boilerplate first, :meth:`~turbohtml.Node.main_text` against resiliparse's
``extract_plain_text(main_content=True)`` and the extractors that expose a main-text mode -- boilerpy3, trafilatura,
justext, newspaper3k, goose3, and news-please. turbohtml scores and extracts in one C pass, leading resiliparse by six
times, boilerpy3 and trafilatura by 51 to 122 times, and the rest by hundreds to over fifteen hundred times.

.. bench-table::
    :file: bench/text-main.json

The annotated pass labels matching elements with spans through :meth:`~turbohtml.Node.to_annotated_text` against
inscriptis's ``get_annotated_text``, 75 times faster; html-text and resiliparse have no annotation surface.

.. bench-table::
    :file: bench/text-annotated.json
...
.. bench-table::
    :file: bench/construct.json

.. bench-table::
    :file: bench/emit.json
...
.. bench-table::
    :file: bench/set-html.json

.. bench-table::
    :file: bench/set-text.json
...
.. bench-table::
    :file: bench/strip-remove.json

.. bench-table::
    :file: bench/strip-tags.json
...
the attribute set in C and leads lxml's like-for-like helpers from low single digits up to over a hundred times, the gap
...
.. bench-table::
    :file: bench/links-extract.json

.. bench-table::
    :file: bench/links-absolutize.json

.. bench-table::
    :file: bench/links-rewrite.json
...
.. bench-table::
    :file: bench/extract-attr.json

.. bench-table::
    :file: bench/extract-text.json
...
to over five hundred times, ``cssmin`` and ``css-html-js-minify`` reaching 2.8 seconds on the 745 kB ``bulma.css`` where
turbohtml takes 5 ms. ``rcssmin`` is a C extension and faster than turbohtml, though it only strips comments and
...
runs about twice slower than turbohtml across the corpus, the cascade pass costing it the most on ``animate.css``, and
it rejects ``foundation.css`` with a parse error on a media query the WHATWG recovery rules accept, where turbohtml
...
structurally before any scoring, which is where the 70x-1700x rows on the pre-declared page come from, and its chardetng
frequency scoring keeps declaration-less single-byte text 3.2x-4.0x ahead of chardet.
...
.. bench-table::
    :file: bench/urls-clean.json

.. bench-table::
    :file: bench/links-filter.json
