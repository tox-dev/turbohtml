...
EXTENSION_DIGITS: Final = frozenset({"123", "4567", "89"})
CARD: Final = re.compile(r"4111 1111 1111 1111")
SLASH_DATE: Final = re.compile(r"(?:[0-3]?\d/[01]?\d|[01]?\d/[0-3]?\d)/(?:[12]\d)?\d{2}")
FULLWIDTH: Final = str.maketrans(string.digits, "".join(chr(0xFF10 + value) for value in range(10)))
ARABIC_INDIC: Final = str.maketrans(string.digits, "".join(chr(0x660 + value) for value in range(10)))
_CARD_SHAPES: Final = ([4, 4, 4, 4], [4, 4, 4, 4, 3], [4, 6, 5], [4, 6, 4])

Found = set[tuple[int, int, str, str]]
...
def renderings(number: phonenumbers.PhoneNumber, region: str) -> list[tuple[str, str]]:
    ...
    forms.extend([
        ...
        ("fullwidth", international.translate(FULLWIDTH)),
        ("arabic-indic", national.translate(ARABIC_INDIC)),
...
def oracle_matches(text: str, region: str | None, leniency: int) -> Found:
    ...
    for match in PhoneNumberMatcher(text, region, leniency=leniency):
        number = match.number
        e164 = phonenumbers.format_number(number, PhoneNumberFormat.E164)
        found.add((match.start, match.end, e164, number.extension or ""))
    return found


def is_luhn_card(text: str) -> bool:
    """Check whether the text's digit run has a Luhn-valid card shape, the shape the recognizer rejects."""
    ascii_text = "".join(str(unicodedata.digit(char)) if char.isdigit() else char for char in text)
    runs = re.findall(r"[0-9]+", ascii_text)
...
def classify(text: str, ours: Found, theirs: Found) -> str:  # ruff:ignore[too-many-return-statements]  # one per category
    ...
    if ours == theirs:
        return "agree"
    if not theirs and ours and SLASH_DATE.search(text):
    ...
    without_extension_digits = {found for found in theirs if text[found[0] : found[1]] not in EXTENSION_DIGITS}
...
def _is_card_shape(text: str, ours: Found, theirs: Found) -> bool:
    ...
    if (
        ...
        return True
    return bool(not ours and theirs and is_luhn_card(text))
